...
check_free_space() {
  ...
  if [ "$AVAILABLE_SPACE_KB" -le "$ONE_GB_IN_KB" ]; then
    ...
    echo "$(date '+%Y-%m-%d %H:%M:%S') | STOP | Free space less than 1GB" >> "$LOG_FILE"
    exit 0
  fi
}
...
create_file() {
  local file_path=$1
  local file_size=$2
  if fallocate -l "${file_size}K" "$file_path"; then
    echo " - A file has been created: $file_path"
    echo "$(date '+%Y-%m-%d %H:%M:%S') | FILE | $file_path | ${file_size}kb" >> "$LOG_FILE"
    return 0
  else
